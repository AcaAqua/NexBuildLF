...
### 2026-06-27 00:00: OSS応募向けガバナンス文書追加

#### 目的
Codex for OSS応募前に、公開リポジトリとして最低限確認しやすいライセンス、セキュリティ、コントリビューション、通知、変更履歴の入口を整える。

#### 変更内容
- ルート直下にMITライセンス文を追加。
- セキュリティ報告方針と公開してはいけない情報の扱いを追加。
- コントリビューション手順、PR確認項目、公開禁止ファイルの注意を追加。
- NOTICEとCHANGELOGを追加。
- READMEから各ガバナンス文書へ移動できる導線を追加。
...
#### 確認結果
- ドキュメント追加のみのため、アプリ実行テストは未実施。
- 追加ファイルの存在とGit差分を確認予定。

#### 残課題
- MIT Licenseを正式採用するかは、メンテナーの最終判断が必要。
- 公開中の`.github/workflows/`、`.vscode/`、`backup-antigravity.patch`を残すか整理するかは別途判断する。

#### 復旧方法
Git管理下の変更のため、必要に応じてこの作業差分を破棄、または該当コミットをrevertする。

### 2026-06-20 17:10: 共有データ取り込み方式の選択追加

#### 目的
受け取った共有データを取り込む前に、既存データを更新するかどうかを現場で判断できるようにし、誤更新・誤復元のリスクを下げる。

#### 変更内容
- 取り込みプレビューに「追加のみ」「追加＋更新」「全復元」の選択肢を追加。
- 既定値を安全側の「追加のみ」に変更。
- 「追加のみ」は既存IDを変更せず、新しい現場・業者だけ追加。
- 「追加＋更新」は同じIDの現場・業者を共有データで更新。
- 「全復元」は全データ共有時のみ表示し、対象範囲を共有データで置き換える。
- 取り込みボタンの文言を選択中の方式に連動。
- 方式ごとの注意文を切り替え、現場で判断しやすくした。
...
- `http://127.0.0.1:3025/settings` のデータタブ表示をBrowserで確認。
- 390px幅の設定データタブで横はみ出しなし、コンソールエラーなしを確認。

#### 残課題
- Browser環境のファイル選択自動操作に制限があるため、実機でJSON選択後に「追加のみ」「追加＋更新」「全復元」の切り替えと取り込み結果を確認する。
- 次段階では、取り込み直後の結果サマリーを画面に残すと、現場で完了確認しやすくなる。
...
### 2026-06-20 23:05: 取り込み後の結果サマリー

#### 目的
共有データの差分確認後、実際に取り込んだ結果をユーザーがすぐ確認できるようにし、追加・更新・保持・削除の判断結果を画面に残す。

#### 変更内容
- 設定画面のデータタブに「直近の取り込み結果」パネルを追加。
- 取り込みモード（追加のみ、差分反映、完全復元）ごとに、現場・協力業者の追加、更新、保持、削除件数を表示。
- 取り込み後にページ再読み込みが走っても結果を確認できるよう、セッション内だけの一時保存を追加。
- 次のファイル選択時には古い取り込み結果を消し、誤認を防ぐようにした。
...
- `npx cap sync android` 成功。
- `android/gradlew.bat assembleDebug` 成功。
- `http://127.0.0.1:3025/settings` のデータタブで共有・取り込み導線の表示を確認。
- デスクトップ幅でコンソールエラーなしを確認。
- モバイル幅で横はみ出しなしを確認。

#### 残課題
- ブラウザ自動確認ではファイル選択ダイアログを直接操作できないため、実ファイル取り込み後の件数表示は手動実機確認を追加で行う。
- 取り込み結果を案件内の共有履歴・工程記録へ残す恒久履歴は未実装。

#### 復旧方法
Git管理下の変更のため、必要に応じて該当コミットまたは作業差分を取り消す。

### 2026-06-20 23:35: 案件ごとの共有・取り込み履歴

#### 目的
LINE、メール、Wi-Fi転送などで案件データをやり取りした後に、どの案件で共有・取り込みが行われたかを工程記録タイムラインから確認できるようにする。

#### 変更内容
- 工程に紐づかない案件全体ログを、既存の `taskLogs` に `share` 種別として保存する方式を追加。
- 案件詳細画面と現場一覧カードの「この案件だけ共有」操作後、対象案件へ共有送信またはファイル保存の履歴を追加。
- 設定画面の全データ共有・現場共有時、対象案件へ一括で共有履歴を追加。
- 共有データ取り込み時、取り込まれる各案件へ取り込み方式、共有範囲、確認コード、元ファイル名を履歴として追加。
- 工程記録タイムラインで案件全体ログを「案件全体」「共有」バッジとして表示。

#### 変更ファイル
- `src/lib/storage.ts`
- `src/lib/projectActivity.ts`
- `src/app/page.tsx`
- `src/app/project/page.tsx`
- `src/app/settings/page.tsx`
- `public/sw.js`
...
- `npm run lint` 成功。
- `npm run build` 成功。
- `http://127.0.0.1:3025/project?id=demo-1` がHTTP 200で応答。

#### 残課題
- ブラウザ自動確認のDOM取得が一時的に空で返ったため、実機または通常ブラウザで共有操作後のタイムライン表示を追加確認する。
- 将来は共有・取り込み履歴だけを絞り込むフィルターを工程記録タブへ追加すると、現場での確認がさらに早くなる。
